...
$search_query = ""; // Initialize search query variable

// Fetch all tires data or search results
try {
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['search_query'])) {
        $search_query = htmlspecialchars($_GET['search_query']);
        $stmt = $conn->prepare("SELECT * FROM tires WHERE item_name LIKE :search_query OR brand LIKE :search_query");
        $stmt->bindValue(':search_query', '%' . $search_query . '%', PDO::PARAM_STR);
    } else {
        $stmt = $conn->prepare("SELECT * FROM tires");
    }
    $stmt->execute();
    $all_tires = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
        ...
        <form class="form-inline my-2 my-lg-0 mr-2" method="GET" action="">
            <input class="form-control mr-sm-2" type="search" placeholder="Search for anything" aria-label="Search" name="search_query" value="<?php echo htmlspecialchars($search_query); ?>">
...
<div class="container mt-4">
    ...
    <?php endif; ?>
    <div class="row">
        <?php if (count($all_tires) > 0): ?>
            <?php foreach ($all_tires as $tire): ?>
