...
try {
    ...
    if (isset($_GET['search'])) {
        ...
        $stmt = $conn->prepare("SELECT * FROM tires WHERE type = 'CHDT' AND (item_name LIKE :search_term OR brand LIKE :search_term)");
        $stmt->bindParam(':search_term', $search_term);
    } else {
        $stmt = $conn->prepare("SELECT * FROM tires WHERE type = 'CHDT'");
    }
    $stmt->execute();
